...
vll mnth{0, 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};

ll days_in_month(ll x) {
	return mnth[x];
}

bool is_valid(ll d, ll m) {
...
ll fun(int x, int y, int z) {
	if(d[x][y][z] != -1) return d[x][y][z];

	fi(0, 3) {
		if(is_valid(x + dx[i], y + dy[i])) {
			d[x][y][z] = !fun(x + dx[i], y + dy[i], z ^ 1);
			if(d[x][y][z]) break;
		}
	}
...
int main()
{
	...
	cin >> d0 >> m0;
	d = vector<vvll>(32, vvll(13, vll(2, -1)));
