...
struct Point{
	...
	Point() {}
	Point(ld a, ld b) {
		// преобразование из углов в координаты
		ld a_rad = to_rad(a);
		ld b_rad = to_rad(b);

		x = R * cos(a_rad) * cos(b_rad);
		y = R * cos(a_rad) * sin(b_rad);
		z = R * sin(a_rad);
		dbg(vd({x, y, z}));
	}
	ld dot(const Point& B) const {
		ld res = x * B.x + y * B.y + z * B.z;
		return res;
	}
	ld len() {
		ld res = sqrt(x * x + y * y + z * z);
		return res;
	}
};
...
void solve() {
	...
	ld alpha = acos(min((ld)1, max((ld)-1, A.dot(B) / (A.len() * B.len()))));
	dbg(alpha);
